...
class OBJECT_OT_GenerateMaze(bpy.types.Operator):
    ...
    # ------------------------------------------------------------------------#
    #                             DEFINE: deselect_objects                    #
    # ------------------------------------------------------------------------#
    #   Deselects all objects in the current Blender view layer.

    #   This method ensures that no objects are selected in the scene
    #   before starting the maze generation, preventing unintended
    #   transformations or deletions of unrelated objects during
    #   the maze creation process.
    #   It is a crucial step to maintain a clean state in the Blender
    #   environment, especially before operations that modify the mesh data.
    ...
    # ------------------------------------------------------------------------#
    #                        DEFINE: delete_existing_maze                     #
    # ------------------------------------------------------------------------#
    #   Deletes the maze object named "Maze" from the current
    #   Blender view layer.

    #   This method ensures that the prior maze (if one exists) is deleted
    #   before generating a new one to reduce clutter and prevent conflicts.
    #   It specifically targets an object named "Maze" and will not delete
    #   objects with different names. Maze.001 will not be deleted for example.
    ...
    def delete_existing_maze(self):
        ...
        try:
            ...
            self.report({"ERROR"}, f"Failed to delete maze: {str(e)}")
            raise
...
class VIEW3D_PT_CreateMazeMenu(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        #   Utilise the wm var and the OGM const to shorten the code here.
        layout.prop(
